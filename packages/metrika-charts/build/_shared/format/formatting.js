...
export const formatForRange = (range) => {
    // Date formatting:
    // Hours: 23:00
    // Days: 25 JUN
    ...
    if (range < hoursCutoff) {
        ...
        return (date) => MONTH_FORMATTER(date).toUpperCase();
    }
};
...
export const bigValueFormatter = (v) => {
    const s = ['K', 'M', 'B'];
...
const pipe = (...fns) => (arg) => fns.reduce((acc, fn) => (fn ? fn(acc) : acc), arg);
// convert microAlgo to Algo
const microAlgoToAlgo = (uAlgo) => uAlgo / 1e6;
// when showing an algo address, only show the first 6 characters
const formatAlgoAddr = (value) => value.substring(0, 6);
// to format algos, use the big value formatter
const formatAlgo = (value) => bigValueFormatter(value);
// to format microalgos, use the big value formatter
const formatMicroAlgo = (value) => bigValueFormatter(value);
export const formatTwoDecimalPlaces = (value) => value.toLocaleString('en-US', { maximumFractionDigits: 2 }); // value.toFixed(2);
export const formatPercent = (value) => d3.format('.2%')(value);
...
export const formattersForTypes = {
 ...
 */
export const formatNumberToFiveCharacters = (v) => {
    ...
    const digitsAfterDecimal = formattedValueWithEnding.split('.').length > 1 ? formattedValueWithEnding.split('.')[1].length : 0;
